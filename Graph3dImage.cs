...
namespace GraphDLL
{
    static class Graph3dImage
    {        
        static Bitmap dest, src;
        static int x0, y0, z0;
        static double AX, BX, CX, AY, BY, CY, _fill_lines;

        internal static void image(int x0, int y0, int z0, int Xsize, int Xfi, int Xteta, int Ysize, int Yfi, int Yteta, Bitmap image)
        {
            //image = Queues.SearchPicture(image, Xsize, Ysize);
            _drawImage(x0, y0, z0, Xsize, Xfi, Xteta, Ysize, Yfi, Yteta, image, Graph.bitmap);
        }

        internal static void _drawImage(int x0, int y0, int z0, int Xsize, int Xfi, int Xteta, int Ysize, int Yfi, int Yteta, Bitmap image, Bitmap dest)
        {
            ...
            Graph3dImage.src = image;
            Graph3dImage.dest = dest;
            Graph3dImage.x0 = x0;
            Graph3dImage.y0 = y0;
            Graph3dImage.z0 = z0;
        ...
        private static void lineImageDraw(double x1, double y1, double x2, double y2, bool Left, double time)
        {
            if (Math.Abs(x2 - x1) > Math.Abs(y2 - y1))
            {
                ...
                for (double i = x1; i < x2; i++)
                {
                    ...
                    double z = (i - x1) * src.Width / (x2 - x1);
                    double k = time * src.Height / _fill_lines;
                    Color c = src.GetPixel((int)z, (int)k);
            ...
            else
            {
                ...
                for (double j = y1; j < y2; j++)
                {
                    ...
                    Color c = src.GetPixel((int)k, (int)z);
                    drawPixel(Left, i, j, c);
                }
            }
        }
